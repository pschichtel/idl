...
typealias ModuleValidator = (Module, List<Module>) -> ValidationResult

private fun detectDuplicatedModules(module: Module, allModules: List<Module>): ValidationResult {
...
object ModuleNameValidator : ModuleValidator {
    ...
    override fun invoke(module: Module, allModules: List<Module>): ValidationResult {
...
private fun detectDuplicateDefinitions(module: Module, allModules: List<Module>): ValidationResult {
...
private fun detectInvalidReferences(module: Module, allModules: List<Module>): ValidationResult {
    ...
    fun validateReference(definition: Definition, ref: ModelReference): List<ValidationError> = buildList {
        ...
        if (definitionLookup == null) {
            add(ValidationError.UndefinedModuleReferenced(module.reference, referencingDefinitionName, referencedModule))
            return@buildList
        }

        val referencedDefinitionName = ref.name
        if (referencedDefinitionName !in definitionLookup) {
            add(ValidationError.UndefinedDefinitionReferenced(module.reference, referencingDefinitionName, referencedModule, referencedDefinitionName))
            return@buildList
        }
    }
    ...
    val errors = module.definitions.flatMap { definition ->
        when (definition) {
            ...
            is Model.HomogenousMap -> validateReference(definition, definition.keyModel) + validateReference(definition, definition.valueModel)
...
private val validations: List<ModuleValidator> = listOf(
    ::detectDuplicatedModules,
    ModuleNameValidator,
    ::detectDuplicateDefinitions,
    ::detectInvalidReferences,
)
...
fun validateModule(module: Module, allModules: List<Module>): ValidationResult {
    return validations.fold<_, ValidationResult>(ValidationResult.Valid) { result, validator ->
        result + validator(module, allModules)
    }
}